...
class FastScroller extends RecyclerView.ItemDecoration implements RecyclerView.OnItemTouchListener {
    ...
    public void onDrawOver(Canvas canvas, RecyclerView parent, RecyclerView.State state) {
        ...
        if (mAnimationState != ANIMATION_STATE_OUT) {
            final FastScrollerAdapter adapter = (FastScrollerAdapter) parent.getAdapter();
            final int topChildPosition = parent.getChildAdapterPosition(parent.getChildAt(0));
            final String title = adapter.getSectionTitle(topChildPosition);

            if (mNeedVerticalScrollbar) {
                drawVerticalScrollbar(canvas, title);
            }
            if (mNeedHorizontalScrollbar) {
                drawHorizontalScrollbar(canvas, title);
            }
        }
    }

    private void drawVerticalScrollbar(Canvas canvas, String title) {
    ...
    private void drawHorizontalScrollbar(Canvas canvas, String title) {
        textView.setText(title.substring(0, 1));
